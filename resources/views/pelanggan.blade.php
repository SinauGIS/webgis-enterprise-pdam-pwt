	...
	<script>
		...
		var pelanggan_dma = L.geoJson(null, {
			style: function (feature) {
				if (feature.properties.jumlah_pelanggan < 2500) {
					return {
						opacity: 1,
						color: 'black',
						weight: 1.0,
				...
				if (feature.properties.jumlah_pelanggan >= 2500 && feature.properties.jumlah_pelanggan <= 5000) {
					return {
						opacity: 1,
						color: 'black',
						weight: 1.0,
				...
				if (feature.properties.jumlah_pelanggan > 5000) {
					return {
						opacity: 1,
						color: 'black',
						weight: 1.0,
			...
			onEachFeature: function (feature, layer) {
				var popup_content = "<h3>" + feature.properties.nama_dma + "</h3>" +
					"<table>" +
					"<tr><th>Kode DMA</th><td>" + feature.properties.kode_dma + "</td></tr>" +
					"<tr><th>Cabang</th><td>" + feature.properties.cabang + "</td></tr>" +
					"<tr><th>Jumlah Pelanggan</th><td>" + feature.properties.jumlah_pelanggan.toLocaleString() + "</td></tr>" +
					"<tr><th>Jumlah Tagihan</th><td>Rp " + feature.properties.tagihan.toLocaleString() + "</td></tr>" +
					"</table>";
				...
				layer.on({
					...
					mouseover: function (e) { //Fungsi ketika mouse di atas obyek
						pelanggan_dma.bindTooltip(feature.properties.nama_dma + " (" + feature.properties.jumlah_pelanggan.toLocaleString() + ")", {
							sticky: true,
							direction: 'top'
						});
					},
		...
		// 			"<table>" +
		// 			"<tr><th>Nomor Pelanggan</th><td>" + feature.properties.nosamw + "</td></tr>" +
		...
		/* Geoserver JSONP */
		var owsrootUrl = 'http://localhost:8080/geoserver/purwokerto/ows';
		var pelanggandmaParameters = {
			service: 'WFS',
			version: '1.0.0',
			request: 'GetFeature',
			typeName: 'purwokerto:jumlah_pelanggan_per_dma',
			outputFormat: 'text/javascript',
			format_options: 'callback:getJson',
			srsName: 'EPSG:4326',
		};
		var pelanggandma_parameters = L.Util.extend(pelanggandmaParameters);
		var pelanggandmaURL = owsrootUrl + L.Util.getParamString(pelanggandma_parameters);
